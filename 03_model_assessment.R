...
####################################################
# Performance assessment with spatial cross-validation
# Case study: landslides in Ecuador
####################################################

# Switch to folder
# with landslides.rds data file:
setwd("data")
...
# Will need a 'data.frame version' of d:
d <- as.data.frame(d)
# and the response should be a factor variable:
...
saveRDS(d, file = "landslides_transformed.rds")
...
# Perform 5-repeated 10-fold non-spatial cross-validation:
...
# Prefer a larger number of repetitions!
...
# Perform non-spatial cross-validation:
res <- sperrorest(formula = fo, data = d, coords = c("x","y"),
                  model_fun = my_gam, 
...
res <- sperrorest(formula = fo, data = d, coords = c("x","y"),
                  ...
                  pred_fun = mypred_rpart, 
                  smp_fun = partition_cv, smp_args = list(repetition = 1:5, nfold = 10))
summary(res$error_rep)[c("train_auroc","test_auroc"),1:2]
# Degree of overfitting (the more negative, the worse):
diff( summary(res$error_rep)[c("train_auroc","test_auroc"),"mean"] )
